...
impl PeerState {
    fn new(heartbeat_registration_data: BTreeMap<String, String>) -> Self {
        Self {
            state: PeerStateType::WaitingCR,
            heartbeat_registration_data,
            challenge: random_string(0x21..=0x7F),
        }
    }
...
fn make_empty_server_datagram(
    message_type: MessageType,
    client_id: u32,
) -> Result<[u8; 7], PackingError> {
...
async fn handle_heartbeat(
    ...
    debug!(
        "GS UDP: Registered client with data: {:?}. Was known before?: {}",
        data, client_was_known
    );
    ...
    if !client_was_known {
        ...
        let ipocts = match peer.ip() {
            ...
            IpAddr::V6(_) => {
                return Err(anyhow!(
                    "Client connected via Ipv6, this is currently not supported."
                ))
            }
        };
        let ipocts_bytes = ipocts
            .into_iter()
            .flat_map(|x| format!("{:02X}", x).as_bytes().to_vec())
            .collect_vec();
        let port_bytes = peer
            .port()
            .to_be_bytes()
        ...
        match srv
            .socket
            .send_to(
                &make_server_datagram(ChallengeResponse, header.client_id, &body)?,
                peer,
            )
            .await
        {
...
async fn handle_challenge_response(
    ...
    if peer_state.state == PeerStateType::WaitingCR {
        ...
        match srv
            .socket
            .send_to(
                &make_empty_server_datagram(ResponseCorrect, header.client_id)?,
                peer,
            )
            .await
        {
...
async fn process_msg(srv: &mut UdpServer) -> Result<(), Error> {
    ...
    let (size, peer) = srv
        .socket
        .recv_from(&mut srv.recv_buf[0..header_len])
        .await?;
    ...
    debug!(
        "GS UDP: Received {:?} from {}/{}: {:02X?} - Bytes remaining: {}",
        header.message_type,
        peer,
        header.client_id,
        &srv.recv_buf[..size],
    ...
    match header.message_type {
        ...
        Enum(KeepAlive) => {
            srv.peers.get(&header.client_id);
        }
