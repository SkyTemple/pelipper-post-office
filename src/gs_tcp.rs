...
enum PeerState {
    WaitingForCr(String), LoggedIn(Session)
}
...
async fn stream_read_single(stream: &mut TcpStream, buf: &mut ReadInBuf, reading_key: bool) -> Result<String, Error> {
...
async fn stream_read_kv(stream: &mut TcpStream, buf: &mut ReadInBuf) -> Result<MaybeFinal<String, String>, Error> {
...
async fn stream_read_collect_payload(stream: &mut TcpStream, buf: &mut ReadInBuf) -> Result<HashMap<String, String>, Error> {
...
async fn send_err(stream: &mut TcpStream, fatal: bool, errmsg: &str, errcode: u32) -> Result<(), Error> {
...
async fn login_peer(server_challenge: &str, stream: &mut TcpStream, buf: &mut ReadInBuf, backends: BackendsRef) -> Result<Session, Error> {
    ...
    if let Some(authtoken) = payload.get("authtoken") {
        if let Occupied(mut sess_entry) = bwrite.sessions.write().await.get_waiting_entry(authtoken.to_string()).await {
            ...
            if let Some(user) = bwrite.users.write().await.get_user_mut(&session.user).await {
                ...
                let expected_gamecode = game_read.game_for_gameid(&user.gs_account.gamecd).await.map(|g| g.code.as_str()).unwrap_or("");
                if payload.get("gamename").map(|x| x.as_str()) == Some(expected_gamecode) {
                    ...
                    if let Some(client_challenge) = payload.get("challenge") {
                        if let Some(response) = payload.get("response") {
                            let expected_response = md5sum(&(
                                md5sum(&session.challenge) + &" ".repeat(48) +
                                &session.token + client_challenge + server_challenge +
                                &md5sum(&session.challenge)
                            ));
                            if response == &expected_response {
                                let proof = md5sum(&(
                                    md5sum(&session.challenge) + &" ".repeat(48) +
                                        &session.token + server_challenge + client_challenge +
                                        &md5sum(&session.challenge)
                                ));
                                // TODO: Seems to have no significance, but maybe it does and we have to also
                                //       update it regularly?
                                let lt = generate_login_ticket();
                                let (userid, profileid, uniquenick) = user.create_or_get_profilemeta().await?;
                    ...
                    debug!("Got game: {:?} -- expected:{}", payload.get("gamename"), expected_gamecode);
            ...
            if validate_profile_id {
                if let Some(strprofileid) = payload.remove("profileid") {
                    ...
                    if user.map(|u| u.profileid) == Some(Some(profileid)) {
                        ...
                        debug!("Expected: {:?}, got {}", user.map(|u| u.profileid), profileid);
...
async fn serve_profile(session: &Session, stream: &mut TcpStream, buf: &mut ReadInBuf, backends: BackendsRef) -> Result<(), Error> {
    ...
    let unqiuenick = validate_profile_request(&mut payload, session, stream, buf, &backends, true).await?;
    let bread = backends.read().await;
    let uread = bread.users.read().await;
...
async fn update_profile(session: &Session, stream: &mut TcpStream, buf: &mut ReadInBuf, backends: BackendsRef) -> Result<(), Error> {
    let mut payload = stream_read_collect_payload(stream, buf).await?;
    let unqiuenick = validate_profile_request(&mut payload, session, stream, buf, &backends, false).await?;
    let bwrite = backends.write().await;
    let mut uwrite = bwrite.users.write().await;
    let user = uwrite.get_user_mut(&unqiuenick).await.unwrap();
    for (k, v) in payload {
        match k.as_str() {
            "lastname" => { user.profile_update_lastname(v).await?; }
...
async fn process_socket(mut stream: TcpStream, backends: BackendsRef) -> Result<(), Error> {
    debug!("GS TCP server; {:?}; connection established.", stream.peer_addr());

    let ch: [char; 10] = random_string('A'..='Z');
    let challenge: String = ch.iter().collect();
    debug!("GS TCP server; {:?}; sending challenge: {}", stream.peer_addr(), challenge);
    ...
    let mut buf = ReadInBuf { unread: 0, cursor: 0, buf: [0; 10240] };
    loop {
        let res = stream_read_kv(&mut stream, &mut buf).await?;
        debug!("GS TCP server; {:?}; command: {:?}", stream.peer_addr(), res);
        match res.as_ref() {
            Msg("login", "") => {
                if let PeerState::WaitingForCr(challenge) = &state {
                    match login_peer(challenge, &mut stream, &mut buf, backends.clone()).await {
                        Ok(session) => {
                            debug!("GS TCP server; {:?}; Successfully logged in.", stream.peer_addr());
                            state = PeerState::LoggedIn(session);
                        },
                        Err(e) => {
                            debug!("GS TCP server; {:?}; Error during login: {} --\n{}", stream.peer_addr(), e, e.backtrace());
                            send_err(&mut stream, false, &e.to_string(), 266).await?;
                        }
                    }
                } else {
                    debug!("GS TCP server; {:?}; Got login request when already logged in.", stream.peer_addr());
            ...
            Msg("getprofile", "") => {
                if let PeerState::LoggedIn(session) = &state {
                    match serve_profile(session, &mut stream, &mut buf, backends.clone()).await {
                        Ok(()) => {
                            debug!("GS TCP server; {:?}; Successfully sent profile.", stream.peer_addr());
                        },
                        Err(e) => {
                            error!("GS TCP server; {:?}; Error sending profile: {} --\n{}", stream.peer_addr(), e, e.backtrace());
                            send_err(&mut stream, false, &e.to_string(), 100).await?;
                        }
                    }
                } else {
                    debug!("GS TCP server; {:?}; Not authenticated for this.", stream.peer_addr());
            ...
            Msg("updatepro", "") => {
                if let PeerState::LoggedIn(session) = &state {
                    match update_profile(session, &mut stream, &mut buf, backends.clone()).await {
                        Ok(()) => {
                            debug!("GS TCP server; {:?}; Successfully updated profile.", stream.peer_addr());
                        },
                        Err(e) => {
                            error!("GS TCP server; {:?}; Error updating profile: {} --\n{}", stream.peer_addr(), e, e.backtrace());
                            send_err(&mut stream, false, &e.to_string(), 100).await?;
                        }
                    }
                } else {
            ...
            Msg("ka", "") => {
                ...
                debug!("GS TCP server; {:?}; Got valid keep-alive.", stream.peer_addr());
            },
            //Msg("lt", lt) => {},
            Final => {
                warn!("GS TCP server; {:?}; Got an unexpected final message.", stream.peer_addr());
                send_err(&mut stream, true, "Invalid request.", 1).await?;
            },
            _ => {
                warn!("GS TCP server; {:?}; Ignoring unknown message '{:?}'.", stream.peer_addr(), res);
                send_err(&mut stream, true, "Invalid request.", 1).await?;
            }
        }
        // wrap cursor around
    ...
    loop {
        ...
        tokio::spawn(async move {
            match process_socket(stream, br).await {
                Ok(_) => {}
                Err(e) => warn!("TCP Client connection closed due to error: {} -- \n{}", e, e.backtrace())
            }
        });
    }
}
